...
export const OAuthContextProvider = (props: { children: JSX.Element }) => {
    ...
    const storeState = (providerId: string, state: string): void => {
        try {
            ...
            console.log(
                `OAuth state for ${providerId} stored in sessionStorage.`
            );
        } catch (error) {
            console.error(
                `Error storing OAuth state for ${providerId}:`,
                error
            );
        }
    };
    ...
    const retrieveState = (providerId: string): string | null => {
        try {
            ...
            console.error(
                `Error retrieving OAuth state for ${providerId}:`,
                error
            );
            return null;
        }
    ...
    const clearState = (providerId: string): void => {
        try {
            ...
            console.log(
                `OAuth state for ${providerId} cleared from sessionStorage.`
            );
        } catch (error) {
            console.error(
                `Error clearing OAuth state for ${providerId}:`,
                error
            );
        }
    };
